...
void MusicTree::ExtractMusicFunctor::operator()() {
    for (auto* reader = tree_ptr->root_element->FirstChildElement("part");
         reader != nullptr &&
         is_element(tree_ptr->valid_part_ids.begin(),
...
std::unique_ptr<Measure> MusicTree::ExtractMusicFunctor::extract_measure(
    ...
    auto meas_uniq_ptr =
        std::make_unique<Measure>(meas_elem_ptr->IntAttribute("number"));

    auto meas_attributes = extract_measure_attributes(meas_elem_ptr);
    for (auto& ptr : meas_attributes) {
        meas_uniq_ptr->add_measure_object(std::move(ptr));
    }

    for (auto reader = meas_elem_ptr->FirstChildElement(); reader != nullptr;
...
MusicTree::ExtractMusicFunctor::extract_note(tx2::XMLElement* note_elem_ptr) {
    ...
    if (tx2::exists(note_elem_ptr, "pitch")) {
        ...
        char pitch_class =
tolower(pitch_elem_ptr->FirstChildElement("step")->GetText()[0]); short
pitch_octave = pitch_elem_ptr->FirstChildElement("octave")->IntText();

        int pitch_alteration = 0;
        if (note_elem_ptr->FirstChildElement("alter") != nullptr) {
            pitch_alteration =
note_elem_ptr->FirstChildElement("alter")->IntText();
        }
    ...
    if (tx2::exists(note_elem_ptr, "grace")) {
        auto grace_before = tx2::exists(note_elem_ptr->PreviousSiblingElement(),
"grace"); auto grace_after = tx2::exists(note_elem_ptr->NextSiblingElement(),
"grace"); auto function = [&](StartStopType type){ auto grace_elem =
tx2::find_element(note_elem_ptr, "grace"); bool is_slashed = false; if
(tx2::attribute_value(grace_elem, "slash") == "yes") { is_slashed = true;
            }
...
the previous one. if (!tx2::exists(note_elem_ptr, "chord") &&
