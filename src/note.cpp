...
Note::Note(const tinyxml2::XMLElement* note_ptr, const MusicTree* tree_ptr) {
    ...
    if (tx2::exists(note_ptr, "type")) {
        this->lly_dur = this->duration_dispatcher.at(tx2::text(note_ptr, "type"))();
    }
    ...
    for (auto i = tx2::find_element(note_ptr, "dot");
         i != nullptr;
    ...
    if (tx2::exists(note_ptr, "pitch")) {
        ...
        this->pitch.pitch_class = std::tolower(tx2::text(pitch_elem_ptr, "step")[0]);
    ...
    if (tx2::exists(note_ptr, "grace")) {
        auto grace_before = tx2::exists(note_ptr->PreviousSiblingElement(), "grace");
