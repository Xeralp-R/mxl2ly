...
void MusicTree::extract_staff_info() {
    auto* scaling_ptr =
        tx2::find_element(this->root_element, "defaults/scaling");

    double staff_width =
        tx2::float_text(tx2::find_element(scaling_ptr, "millimeters"));
    double tenths_in_staff =
        tx2::float_text(tx2::find_element(scaling_ptr, "tenths"));
...
void MusicTree::extract_header_block() {
    ...
    for (tx2::XMLElement* runner =
             this->root_element->FirstChildElement("credit");
         runner != nullptr;
...
void MusicTree::extract_music() {
    for (auto* reader = root_element->FirstChildElement("part");
         reader != nullptr &&
         helper::is_element(valid_part_ids.begin(), valid_part_ids.end(),
                            reader->Attribute("id"));
